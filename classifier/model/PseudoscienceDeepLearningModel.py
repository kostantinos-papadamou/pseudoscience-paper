...
class PseudoscienceDeepLearningModel(object):
    ...
    def build_model(self):
        ...
        if self.CONFIG.INPUT_FEATURES_CONFIG['video_snippet']:
            ...
            # Add Input Layer
            video_snippet_model_branch.add(InputLayer(input_shape=(self.CONFIG.EMBESSING_DIM,), name='video_snippet_input'))
        ...
        if self.CONFIG.INPUT_FEATURES_CONFIG['video_tags']:
            ...
            # Add Input Layer
            video_tags_model_branch.add(InputLayer(input_shape=(self.CONFIG.EMBESSING_DIM,), name='video_tags_input'))
        ...
        if self.CONFIG.INPUT_FEATURES_CONFIG['video_transcript']:
            ...
            # Add Input Layer
            video_transcript_model_branch.add(InputLayer(input_shape=(self.CONFIG.EMBESSING_DIM,), name='video_transcript_input'))
        ...
        if self.CONFIG.INPUT_FEATURES_CONFIG['video_comments']:
            ...
            # Add Input Layer
            video_comments_model_branch.add(InputLayer(input_shape=(self.CONFIG.EMBESSING_DIM,), name='video_comments_input'))
        ...
        # Add Dropout Layer
        dropout_layer_1 = Dropout(rate=self.CONFIG.MODEL_DROPOUT, name='dropout_layer_1')(fully_connected_1)

        """ 2nd Fully Connected (Dense) Layer """
        # Add Fully Connected Layer
        fully_connected_2 = Dense(units=128, activation='relu', name='fully_connected_2')(dropout_layer_1)
        # Add Dropout Layer
        dropout_layer_2 = Dropout(rate=self.CONFIG.MODEL_DROPOUT, name='dropout_layer_2')(fully_connected_2)

        """ 3rd Fully Connected (Dense) Layer """
        # Add Fully Connected Layer
        fully_connected_3 = Dense(units=64, activation='relu', name='fully_connected_3')(dropout_layer_2)
        # Add Dropout Layer
        dropout_layer_3 = Dropout(rate=self.CONFIG.MODEL_DROPOUT, name='dropout_layer_3')(fully_connected_3)

        """ 4th Fully Connected (Dense) Layer """
        # Add Fully Connected Layer
        fully_connected_4 = Dense(units=32, activation='relu', name='fully_connected_4')(dropout_layer_3)
        # Add Dropout Layer
        dropout_layer_4 = Dropout(rate=self.CONFIG.MODEL_DROPOUT, name='dropout_layer_4')(fully_connected_4)

        """ Classification Layer (Softmax) """
        # Add Classification (Softmax) Layer
        classification_layer = Dense(units=self.CONFIG.NB_CLASSES, activation='softmax', name='classification_layer')(dropout_layer_4)
        ...
        # Compile the Model
        full_deep_learning_model.compile(loss=self.CONFIG.LOSS_FUNCTION, optimizer=optimizer, metrics=['accuracy'])
        # Summarize the Model
